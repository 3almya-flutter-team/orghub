...
class AllAdsCommentsView extends StatefulWidget {
  final int advertId;

  const AllAdsCommentsView({Key key, this.advertId}) : super(key: key);
  @override
  _AllAdsCommentsViewState createState() => _AllAdsCommentsViewState();
}

class _AllAdsCommentsViewState extends State<AllAdsCommentsView> {
  GetAllAdsCommentsBloc getAllAdsCommentsBloc =
      kiwi.KiwiContainer().resolve<GetAllAdsCommentsBloc>();

  @override
  void initState() {
    getAllAdsCommentsBloc
        .add(GetAllAdsCommentsEventsStart(advertId: widget.advertId));
    super.initState();
  }

  @override
  void dispose() {
    getAllAdsCommentsBloc.close();
    super.dispose();
  }
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: appBar(
            context: context,
            title:
                translator.currentLanguage == "en" ? "Ads comments" : "التعليقات على الاعلان",
            leading: true),
        body: BlocBuilder(
          bloc: getAllAdsCommentsBloc,
          builder: (context, state) {
            if (state is GetAllAdsCommentsStatesStart) {
              return SpinKitThreeBounce(
                color: AppTheme.primaryColor,
                size: 30,
              );
            } else if (state is GetAllAdsCommentsStatesSuccess) {
            ...
            } else if (state is GetAllAdsCommentsStatesFailed) {
              if (state.errType == 0) {
